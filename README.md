...
## Podverse First Submission Checklist

Before continuing beyond the seed-registration gate in the rollout plan, submit and merge a Podverse registry PR with:

- `registry/apps/podverse.app.json`
- Podverse metadata: `app_id`, `display_name`, `owner`, `status`, `created_at`, `updated_at`
- First signing key in `signing_keys[]`
- Passing `validate-registry` check on the PR
